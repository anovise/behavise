...
Behavier gives you first-class trackers for the most common UX analytics needs — pointer movement, dwell time, scroll depth, page visits, click counts, and element visibility — all with a consistent API, zero runtime dependencies, and full TypeScript support.

[![npm](https://img.shields.io/npm/v/behavier)](https://www.npmjs.com/package/behavier)
[![license](https://img.shields.io/github/license/reasvyn/behavier)](LICENSE)
...
| Package                                         | Description                                                                             |
| ----------------------------------------------- | --------------------------------------------------------------------------------------- |
| [`@behavier/core`](packages/core)               | Base types, `BaseTracker`, `EventDispatcher`, `MemoryAdapter`, `LocalStorageAdapter`    |
...
### `createBehavier(options?)` — `behavier`
...
// Shared adapter across all trackers (via createBehavier)
const b = createBehavier({ storage: new LocalStorageAdapter('analytics') })
```
...
Behavier is written in strict TypeScript. All event payloads are fully typed:
...
**Browser support:** Behavier targets browsers supporting ES2017+ (Chrome 58, Firefox 52, Safari 10.1, Edge 16). `VisibilityTracker` additionally requires `IntersectionObserver` support (all modern browsers; polyfill available).
...
│   └── behavier/         # behavier (main entry + factory)
├── turbo.json
├── tsconfig.base.json
└── package.json
```
